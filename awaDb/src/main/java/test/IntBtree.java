...
public class IntBtree extends AbstractBTree{
    public IntBtree(int maxItemCount, int pageSize,Function<Long, OffHeapMemory> memoryProvider) {
    ...
    protected void fillIdxNode(long dstNodeIdx, long dstItemIdx, MemHandler itemHandler, long srcNodeIdx, long srcItemIdx) {
        int key = itemHandler.getInt(srcNodeIdx,srcItemIdx);
    ...
    public final Iterator findEq(Iterator ite,int key) {
    ...
    public Iterator findGE(Iterator ite,int key) {
        int leafNodeIdx = findLeafNode(0, 0, key);
        int size = Math.min(this.currentItemIndex - (leafNodeIdx * this.keyNodeCapacity), this.keyNodeCapacity);
    ...
    public IntEntry getEntry(int index, IntEntry entry) {
        entry.with(getKey(index), getValue(index));
        return entry;
    }
    
    private int getKey(MemHandler itemHandler,int nodeIdx,int itemIdx)
    {
        return (int)itemHandler.getInt(nodeIdx,itemIdx);
    }
    
    private int adjustEqLowPos(int pos, int matchKey) {
    ...
    private int adjustEqUpperPos(int pos, int matchKey) {
    ...
    public int getKey(int index) {
        ...
        return this.keyItemHandler.getInt(nodeIdx, itemIdx);
    }
    
    protected int binarySearch(MemHandler itemHandler, int nodeIdx, int size, int key) {
        int low = 0;
        int high = size - 1;
    ...
    private int findLeafNode(int level, int itemIdx, int key) {
    ...
    public static void main(String[] args) {
        ...
        IntBtree btree = new IntBtree(vol, 4096, layout -> man.allocateNative(layout));
        ...
        while (j < 100000) {
            ...
            k = 0;
            while (k < vol) {
                Iterator ite2 = btree.findEq(ite,k);
