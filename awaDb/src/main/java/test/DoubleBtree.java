...
public class DoubleBtree extends AbstractBTree {
    ...
    private int maxExp;
    public DoubleBtree(int maxItemCount, int pageSize, Function<Long, OffHeapMemory> memoryProvider) {
    ...
    protected void fillIdxNode(long dstNodeIdx, long dstItemIdx, MemHandler itemHandler, long srcNodeIdx, long srcItemIdx) {
        ...
        if (itemHandler == keyItemHandler) {
            long mantissa = (long) itemHandler.getLong(srcNodeIdx, srcItemIdx);
            int exp = (int) ((long) getRowValue(((int)srcNodeIdx) * this.keyNodeCapacity + (int)srcItemIdx));
            key = mantissa * NumberQueryBlock.lookupTable[exp];
        } else {
            key = (double) itemHandler.getDouble(srcNodeIdx, srcItemIdx);
        }
    ...
    public void put(JsonNumber key, long value) {
        ...
        this.keyItemHandler.set(keyNodeIdx, KeyitemIdx, key.mantissa());

        if (this.currentItemIndex == 0) {
            this.minKeyHandler.set(key.mantissa() * NumberQueryBlock.lookupTable[exp]);
        ...
        if (this.currentItemIndex == this.maxItemCount) {
            this.maxKeyHandler.set(key.mantissa() * NumberQueryBlock.lookupTable[exp]);
        }
    }
    ...
    protected final void setValue(long value) {
        ...
        this.valueItemHandler.set(valueNodeIdx, valueitemIdx, value);
    }
    ...
    public long getValue(int index) {
        ...
        return (long) getStartingOffset() + ((int) (this.valueItemHandler.getLong(nodeIdx, itemIdx) >> 32));
    }
    
    @Override
    public void complete() {
       this.maxExpHandler.set((long)maxExp);
       super.complete();
    }
    
    @Override
    public long getMaxKeyAsLong() {
        return (long) this.maxKeyHandler.getLong();
    }

    @Override
    public long getMinKeyAsLong() {
        return (long) this.minKeyHandler.getLong();
    }
    ...
    private double getKeyAsDouble(MemHandler itemHandler, int nodeIdx, int itemIdx) {
        return itemHandler.getDouble(nodeIdx, itemIdx);
    }
    ...
    public int compare(int index, long rightMantissa, int rightExp) {
        ...
        return Double.compare(left * NumberQueryBlock.lookupTable[leftExp], rightMantissa * NumberQueryBlock.lookupTable[rightExp]);
    }

    protected int binarySearch(MemHandler itemHandler, int nodeIdx, int size, JsonNumber key) {
       if(itemHandler == keyItemHandler)
    ...
    private int binaryLongSearch(MemHandler itemHandler, int nodeIdx, int size, JsonNumber key) {
        int low = 0;
        int high = size - 1;
        ...
        while (low <= high) {
            ...
            int  exp = (int)(valueItemHandler.getLong((nodeIdx * this.keyNodeCapacity) >> this.valueExponent,0));
    ...
    private int binaryDoubleSearch(MemHandler itemHandler, int nodeIdx, int size, JsonNumber key) {
        int low = 0;
        int high = size - 1;
        int exp = Math.abs(key.exp());
        while (low <= high) {
            ...
            int ret = Double.compare(midVal, key.mantissa() * NumberQueryBlock.lookupTable[exp]);
            if (ret < 0) {
                low = mid + 1;
            } else if (ret > 0) //else if (midVal > key) 
            {
