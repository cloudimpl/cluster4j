...
public class ShortBtree extends AbstractBTree{
    ...
    protected void fillIdxNode(long dstNodeIdx, long dstItemIdx, VarHandle itemHandler, long srcNodeIdx, long srcItemIdx) {
        short key = (short) itemHandler.get(address,srcNodeIdx,srcItemIdx);
    ...
    {
        return (short)this.maxKeyHandler.get(this.address);
    }
    
    @Override
    public long getMinKeyAsLong()
    {
        return (short)this.minKeyHandler.get(this.address);
    }
    ...
    private short getKey(VarHandle itemHandler,int nodeIdx,int itemIdx)
    {
        return (short)itemHandler.get(this.address,nodeIdx,itemIdx);
    }
    ...
    public short getKey(int index) {
        ...
        return (short) this.keyItemHandler.get(this.address, nodeIdx, itemIdx);
    }
    
    protected int binarySearch(VarHandle itemHandler, int nodeIdx, int size, short key) {
        int low = 0;
        int high = size - 1;
