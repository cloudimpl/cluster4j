...
public class ByteBtree extends AbstractBTree {
    ...
    protected void fillIdxNode(long dstNodeIdx, long dstItemIdx, VarHandle itemHandler, long srcNodeIdx, long srcItemIdx) {
        byte key = (byte) itemHandler.get(address, srcNodeIdx, srcItemIdx);
    ...
    private byte getKey(VarHandle itemHandler, int nodeIdx, int itemIdx) {
        return (byte) itemHandler.get(this.address, nodeIdx, itemIdx);
    }
