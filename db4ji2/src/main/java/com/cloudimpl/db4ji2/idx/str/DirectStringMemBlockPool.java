...
/** @author nuwansa */
public class DirectStringMemBlockPool implements StringMemBlockPool{
  private final int poolMemSize;
  private final int pageSize;
  private final ManyToManyConcurrentArrayQueue<StringMemBlock> memPool;
  private final ByteBuffer buffer;
  private final LongBuffer longBuffer;
  public DirectStringMemBlockPool(int poolMemSize, int pageSize) {
  ...
  private void allocate() {
    ...
    int i = 0;
    while (i < blocks) {
      memPool.add(new StringMemBlock(buffer,new DirectStringBlock(pageSize), i * pageSize, pageSize,()->new StringEntry()));
      i++;
    }
  }
