...
public class PageBlock2 {
  ...
  public static final class Page {
    ...
    public long findGt(long key) {
      if (readOnly) {
        return findGtReadOnly(key);
      } else {
        return findGtWritable(key);
      }
    }
    ...
    private long findGtReadOnly(long key) {
      int idx =
          Arrays.binarySearch(
              this.items.array(),
              this.items.arrayOffset(),
              ...
              key);
      if (idx >= 0) {
        while (idx < items.position()) {
          if (items.array()[idx] != key) return items.array()[idx];
          else idx++;
        }
      }
      return -1;
    ...
    private long findGtWritable(long key) {
